function fixed_effects_ALL( subjspat )
models = { 'MOV*' };
...
for m=1:length(models)
    ...
    destdir = fullfile( stats_dir, 'FIXED_EFFECT', 'ALL' );
    fprintf('OUT DIR - %s\n', destdir);
    
    %Defining subjs
    [~, dirs] = system( ['find ' datadir ' -name "' subjspat '" -type d | sort'] );
    dirs = regexprep(dirs,'\n*$', '');
    subjs = regexp(dirs,'\n', 'split');
    ...
    for s=1:8
        ...
        %preparing data
        tmp = load( fullfile( subj, 'BATCH_1_FIRST_LEVEL.mat' ) );
        if s == 1
    ...
    tmp = load( fullfile( firstsubj, 'BATCH_2_CONTRAST.mat' ) );
