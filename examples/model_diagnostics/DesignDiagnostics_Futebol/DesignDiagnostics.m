function [simdata,multicol]=DesignDiagnostics(X,C,tr, convolved, X_names,tit)
%%
...
b_true=randi([5 10+size(X,2)],1,size(X,2));     % Pick some random numbers for betas
...
for i=1:nloops
    ...
    %-------------------------------------------------------
    noise=randn(size(X,1),1)*noise_multip;             % Noise for the regression
...
figure; % Regressor correlations (with numbers) 
    imagesc(corr(Xmatrix),[-1 1]);colormap(redblue);set(gca,'xticklabel',[],'ytick', 1:length(X_names),'yticklabel',X_names);title(sprintf('Regressor correlation\n%s',tit));colorbar;
...
%    textStrings=num2str(textStrings(:),'%0.2f'); % Create strings from the correlation matrix
    ...
    figure
    subplot(2,2,[1:2]);%plot(zscore(Xmatrix));xlabel('TR');ylabel('Regressor Z-scored amplitude');set(gca,'ytick',[]);title('Design');legend(leg);
    N = size(Xmatrix,1);
    plot(1:N/2,Xmatrix(1:N/2,:));xlabel('TR');ylabel('Regressor original amplitude');set(gca,'ytick',[]);title(tit)
    
    subplot(2,2,[3:4]);%plot(zscore(Xmatrix));xlabel('TR');ylabel('Regressor Z-scored amplitude');set(gca,'ytick',[]);title('Design');legend(leg);
    plot(N/2+1:N,Xmatrix(N/2+1:end,:));xlabel('TR');ylabel('Regressor original amplitude');set(gca,'ytick',[]);title(tit);legend(X_names);
