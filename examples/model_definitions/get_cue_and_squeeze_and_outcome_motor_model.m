...
function model = get_cue_and_squeeze_and_outcome_motor_model()
    ...
    model.contrast(ci).name = 'CSO FLA > CSO STI';
    
    ci= ci+1;
    
    model.contrast(ci).vec  = [ -1 1 0];
    model.contrast(ci).name = 'CSO STI > CSO FLA';
    
    ci= ci+1;
       
    model.contrast(ci).vec  = [ 1 0 -1];
    model.contrast(ci).name = 'CSO FLA > CSO SELF';

    ci= ci+1;
       
    model.contrast(ci).vec  = [ -1 0 1];
    model.contrast(ci).name = 'CSO SELF > CSO FLA';
    ...
    model.contrast(ci).name = 'CSO FLA > CSO EFFORT';

    ci = ci+1;
    model.contrast(ci).vec  = [ 0 1 0 -1];
    model.contrast(ci).name = 'CSO STI > CSO EFFORT';
    
    ci= ci+1;
    
    model.contrast(ci).vec  = [ 0 0 1 -1];
    model.contrast(ci).name = 'CSO SELF > CSO EFFORT';
    ...
    model.contrast(ci).name = 'CSO FLA > CUEONLY FLA';
